...
用 **`scripts/generate_alipay_rsa2_keys.sh`** + [reference.md](reference.md) 完成 **本地生成 → 控制台上传公钥或 CSR → 配置 env/SDK → 加签与回调验签**；**不依赖**桌面密钥工具或 MCP。必须在 [开放平台](https://open.alipay.com) **开发信息** 中绑定材料并获取 **支付宝公钥/证书**（业务步骤，非脚本可代替）。
...
- 生成或轮换 RSA2 材料、区分公钥/证书模式、排查验签与密钥不匹配。
...
```text
- [ ] OpenSSL 可用；已用脚本生成材料（或按 reference 手写等价命令）
- [ ] 控制台已绑定公钥或已完成证书流程
- [ ] 支付宝侧验签材料与 APPID、环境（正式/沙箱）一致
- [ ] 密钥未进入版本库
```
